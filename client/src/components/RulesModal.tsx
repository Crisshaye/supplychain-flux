  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div>
          <h3 className="text-sm font-display font-semibold mb-3 text-text">
            Parameters
          </h3>
...
function Row({ name, sym, value, note }: { name: string; sym: string; value: string; note: string }) {
  return (
    <tr>
      <td className="px-4 py-2 text-text">{name}</td>
      <td className="px-4 py-2 text-text-muted font-mono text-xs">{sym}</td>
      <td className="px-4 py-2 text-right num text-text font-medium">{value}</td>
